...
> **Status.** The hackathon is over and the submission is made. This backlog is
> now a single-owner engineering list; the deadline framing and the per-person
> allocation have been removed. 351 tests pass and `bash scripts/validate.sh`
> passes every check against a live cluster.
>
> The one open blocker is reasoning credentials: watsonx needs its project
> associated with the active WML instance, and the IBM Bob API base URL is
> still unknown. See `SHIVRAJ_DOCS/01_IBM_BOB_SETUP.md`.
...
- [ ] **BOB-001** — Obtain IBM Bob credentials; observe one real analysis
  ...
  -· *Depends:* — · *Est:* 30 min (unbounded if the endpoint is wrong)
...
- [ ] **SUB-001** — Written problem and solution statements *(45 min)*
- [ ] **SUB-002** — Written statement on how IBM Bob was utilised *(45 min)*
- [ ] **SUB-003** — Exported IBM Bob report of all relevant tasks/sessions *(30 min)*
- [ ] **SUB-004** — Demo video including how IBM Bob was used, in English *(2 h)*
- [ ] **SUB-005** — Full-history secret sweep, then tag `v1.0-submission` *(30 min)*
...
- [x] **MCP-005** — Import `Enum` in `mcp_server/tickets.py`
  ...
  -· *Est:* 5 min · *Files:* `mcp_server/tickets.py`
...
- [x] **MCP-001** — Rename 3 MCP tools to the names both consumers expect
  ...
  -· *Est:* 20 min · *Commit:* `fix(mcp): align tool names`
...
- [x] **MCP-003** — Move `evidence.py` into `mcp_server/`; delete `orchestrator/`
  -· *Est:* 20 min · *Done:* `git grep "from orchestrator"` empty
...
- [x] **MCP-008** — Adapter: MCP evidence + SQLite tickets to agent contracts
  ...
  -· *Est:* 1.5 h · *Files:* new `agent/adapters.py`
...
- [x] **EXEC-001** — Real `KubernetesClient`
  ...
  -· *Est:* 1.5 h · *Files:* new `agent/k8s_client.py`
...
- [ ] **DASH-002** — Reject dialog requiring a reason *(1 h)*

---

## P2 — quality and reliability

- [x] **REPO-001** — Untrack `data/kubemedic.db`; ignore `data/*.db`, `records/*.json` *(10 min)*
...
- [x] **REPO-002** — Root `requirements.txt` and `requirements-dev.txt` *(15 min)*
...
- [x] **MCP-007** — Tool errors surface as errors, not as successful text *(20 min)*
- [x] **MCP-004** — Add `get_recent_changes` to the `alwaysAllow` list *(5 min)*
...
- [x] **TEST-001** — Ticket store unit tests on a temp database *(45 min)*
- [x] **TEST-002** — MCP profile tests *(30 min)* — folded into MCP-002
- [ ] **TEST-003** — `_extract_json` parsing tests *(30 min)*
- [ ] **TICKET-002** — Incident state propagates to member tickets *(45 min)*
- [x] **E2E-001** — Rewrite `scripts/validate.sh` as a real harness *(1.5 h)*
- [ ] **DASH-003** — Remove Gemini strings from user-visible surfaces *(15 min)*
...
- [ ] **DOC-001** — Update `docs/consolidation-inventory.md` *(10 min)*
